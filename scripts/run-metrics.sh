...
# Project-local fallback (owner-map.json permits project-local fixes for
# recurring template bugs, folded back via postmortem): this project's real
# AC tests predate the scaffold and are named tests/ac<NN>_*.rs +
# tests/kind_conformance.rs (one integration-test BINARY per AC, not a
# tests/acceptance_<id>.rs function), so the acceptance_* count above is
# always 0 here. When that's the case, count AC-binary pass/fail instead:
...
if [ "$AC_TOTAL" -eq 0 ]; then
  ...
  if [ "$PROJECT_AC_TOTAL" -gt 0 ]; then
    PROJECT_AC_PASSING=$(awk -v aclist="$PROJECT_AC_FILES" '
      ...
      /^test result: ok\./ {
        if (cur != "" && (cur in want)) passing[cur] = 1
        cur = ""
      }
