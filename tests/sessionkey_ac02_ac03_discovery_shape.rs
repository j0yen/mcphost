...
async fn every_host_star_descriptor_declares_an_optional_tenant_key() {
    ...
    assert_eq!(
        ...
        "the thirteen host.* control tools (incl. host.quickstart, host.tool_run, and \
         ...
         (PRD-mcphost-tenant-state) plus the six host.table.* tools \
         (PRD-mcphost-tenant-tables) plus the eight host.tool_share/host.tool_unshare/ \
         ...
         four host.agent.* tools (PRD-mcphost-agent-directory) plus the seven \
         host.msg.* tools (PRD-mcphost-agent-inbox) plus the seven \
         host.agent.contact_*/mute/unmute tools (PRD-mcphost-agent-consent): {entries:?}"
    );
