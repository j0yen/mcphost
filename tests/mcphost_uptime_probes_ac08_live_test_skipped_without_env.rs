...
//! P1 requirement 7 / AC8 — Given `MCPHOST_LIVE` unset, When the test
...
//! `tests/mcphost_team_memory_ac08_live_test_skipped_without_env.rs`.
...
async fn live_proof_against_real_mcphost_when_enabled() {
    ...
    let script =
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("examples/uptime-probes/proof.sh");
    let mut cmd = Command::new("bash");
    cmd.arg(&script);
    if let Ok(url) = std::env::var("MCPHOST_URL") {
        cmd.env("MCPHOST_URL", url);
    }
    let output = cmd.output().await.expect("run examples/uptime-probes/proof.sh");
