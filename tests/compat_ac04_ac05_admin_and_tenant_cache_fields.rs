...
async fn ac5_fresh_tenant_tools_list_has_cache_fields() {
    ...
    assert_eq!(
        names.len(),
        47,
        "there must be exactly the fourteen host.* control-plane tools \
         (incl. host.quickstart, host.tool_run, host.bridge_test, and host.spec_test -- \
         PRD-mcphost-tool-test) plus host.tool_call plus the nine host.state.* tools \
         ...
         host.runs.* tools (PRD-mcphost-runs-and-jobs) plus the seven host.trigger.* \
         tools (PRD-mcphost-schedules) plus the three billing.* \
         tools: {names:?}"
    );
}
