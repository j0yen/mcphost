...
async fn ac5_fresh_tenant_tools_list_has_cache_fields() {
    ...
    assert_eq!(
        ...
        "there must be exactly the sixteen host.* control-plane tools \
         ...
         PRD-mcphost-handoff-token) plus host.tool_call plus the nine host.state.* tools \
         ...
         seven host.agent.contact_*/mute/unmute tools (PRD-mcphost-agent-consent): {names:?}"
    );
}
