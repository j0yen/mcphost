...
async fn ac1_anonymous_tools_list_has_cache_fields() {
    ...
    assert_eq!(
        ...
        "signup + host.* (incl. host.quickstart, host.tool_run, and host.bridge_test) + \
         ...
         PRD-mcphost-sharing) + host.share.caller_limit/caller_limit_remove \
         (2 tools, PRD-mcphost-shared-tool-caller-usage) + host.runs.* (5 tools, PRD-mcphost-runs-and-jobs) + \
         host.progress/host.runs.part (2 tools, PRD-mcphost-run-result-overflow-to-state) + \
