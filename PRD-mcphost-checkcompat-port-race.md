...
- deferred_acs: [8]
- mock_justifications: AC8 — this coding sandbox has no HCLOUD_TOKEN, so burst-lane/hcloud cannot authenticate to boot a ccx53 box (Hetzner's API itself is reachable: a live probe returns HTTP 401, not a network failure); burst-lane also shows no active session, and its installed CLI (status/cost/why-down/sub-cap/route-check/evidence/keep-check) has no `prove` verb to run even if a box were up. This does not override the Operator-authorization line above (which stays in force whenever a credentialed box is actually reachable) — it is inapplicable to this specific dispatch because there is nothing here to act on it with. The port-race concurrency this AC would exercise on real hardware is already covered locally by AC5's 200-iteration `--test-threads=32` stress run on the remote build runner. Re-run `burst-lane prove` (or its current prove-equivalent) for real and update this line the next time a credentialed burst box is reachable from wherever this PRD's HEAD is being verified.
